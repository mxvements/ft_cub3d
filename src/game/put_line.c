...
static float whileTouch(t_player *player, t_cub *cub, float cos_angle, float sin_angle, int color)
{
    ...
    float   ray_col;
    image.img = NULL;
    ray_row = (float)player->win_row;
    ray_col = (float)player->win_col;
    // FOV on minimap
    ...
    //perspective
    return distance(ray_row, player->win_row, ray_col, player->win_col, cub);
}
...
{
	t_player	*player;
	const float	fov = cub->options.fov;
	float		fraction;
	float		start_angle;
	int			screen_col_idx;
